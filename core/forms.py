...
def get_validate(address):
	"""
	Validate email using mailgun validation (to be used on top of normal django evaluation).
	"""	
	# return requests.get("https://api.mailgun.net/v3/address/validate",auth=("api",settings.MAILGUN_API_KEY),params={"address": address})
...
class UserForm(forms.ModelForm):
	"""
	First half of registration form, this is the User part plus birthday Event object data (created in view).
	"""
	...
	password2 = forms.CharField(required=True,label=_("Password confirmation"),widget=forms.PasswordInput,
...
class UserProfileForm(forms.ModelForm):
	"""
	Second half of registration form, the UserProfile part.
	"""
...
class UserEditForm(forms.ModelForm):
	"""
	First part of Edit form, User part (without birthday Event).
	"""
...
class UserProfileEditForm(forms.ModelForm):
	"""
	Second part of Edit form, UserProfile part.
	"""
...
class BirthdayEditForm(forms.ModelForm):
	"""
	Third part of Edit form: modelform specifically for already created birthday type Events.
	"""
	class Meta:
		model = Event
